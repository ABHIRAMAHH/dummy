...
public class SJFScheduler {
    ...
    /**
     * SJF: sort by shortest job first (estimated exec time on a reference VM), then bind greedily
     * to the VM that gives earliest completion (based on per-VM ready time).
     */
    public static List<CloudletSimple> schedule(
        ...
        // reference speed: fastest VM capacity
        double refCap = vms.stream().mapToDouble(SchedulerUtil::capacityMiPerSec).max().orElse(1.0);
