...
public class PriorityScheduler {
    ...
    private static double estimatedExecSec(WorkloadLoader.WorkloadTask t, Vm vm) {
        double length = Math.max(100.0, (t.taskExecutionTime * t.cpuUtilization) / 50.0);
        return length / Math.max(1.0, vm.getMips());
    }
    ...
    public static List<CloudletSimple> schedule(
        ...
        Map<Vm, Double> backlogMi = new HashMap<>();
        Map<Vm, Double> lastUpd   = new HashMap<>();
        for (Vm vm : vms) { backlogMi.put(vm, 0.0); lastUpd.put(vm, 0.0); }

        List<CloudletSimple> cloudlets  = new ArrayList<>();
        Map<Long, Integer>   bindCounts = new TreeMap<>();
        double[] backlogSec = new double[V];
        ...
        for (WorkloadLoader.WorkloadTask t : tasks) {
            ...
            try {
                if (predictor != null)
                    predTh = predictor.getPredictedThroughput(arrival);
            } catch (Throwable ignored) {}
            ...
            Vm     bestVm          = null;
            double bestObjective   = Double.MAX_VALUE;
            double bestExpCost     = Double.MAX_VALUE;
            double bestCompletion  = Double.MAX_VALUE;
            int    bestIdx         = 0;
            ...
            for (int k = 0; k < V; k++) {
                ...
                double newBacklog = backlog + lengthMi;
                if (newBacklog / capacity > 200) continue;

                double loadFactor   = newBacklog / capacity;
                double loadPenalty  = Math.pow(loadFactor, 1.5);
                double completion   = arrival + (newBacklog / capacity) + loadPenalty;

                double baseCost     = costPerSecond(vm);
                double adjCost      = baseCost / (1.0 + predTh / 10.0);
                double costPerMi    = adjCost / capacity;

                double cpuUtil = 0.5;
                try {
                    if (vm.getHost() != null)
                        cpuUtil = Math.max(0.01, vm.getHost().getCpuMipsUtilization());
                } catch (Throwable ignored) {}

                double expectedCost = lengthMi * costPerMi * (1.0 + 0.3 * cpuUtil);
                double objective    = completion + LAMBDA_COST * (expectedCost / 100.0)
                        + Math.random() * 0.01;

                int assigned = bindCounts.getOrDefault(vm.getId(), 0);
                if (assigned > tasks.size() / V + 50) continue;
                objective += assigned * 0.2;

                if (objective < bestObjective - EPS
                        || (Math.abs(objective - bestObjective) <= EPS
                        && expectedCost < bestExpCost - EPS)
                        || (Math.abs(objective - bestObjective) <= EPS
                        && Math.abs(expectedCost - bestExpCost) <= EPS
                        && completion < bestCompletion - EPS)) {
                    bestObjective  = objective;
                    bestExpCost    = expectedCost;
                    bestCompletion = completion;
                    bestVm         = vm;
                    bestIdx        = j;
                }
            ...
            backlogMi.put(bestVm, backlog + lengthMi);
            lastUpd.put(bestVm, arrival);
            backlogSec[bestIdx] += lengthMi / capacity;

            if ((scheduled % ROTATE_EVERY_TASKS) == 0)
                offset = (offset + 1) % V;
        }

        printSummary("Priority", bindCounts, vms, backlogSec);
        // at the end of each scheduler, after printSummary()
        ResultsExporter.addResult(
                buildResult("Priority", bindCounts, vms, backlogSec, -1, -1, null)
        );
        broker.submitCloudletList(cloudlets);
        return cloudlets;
    }
