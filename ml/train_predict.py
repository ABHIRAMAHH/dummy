...
def fit_transform_scalers_no_leakage(X_train_raw, X_test_raw, y_train_raw, y_test_raw):
    """
    X_*_raw: (samples, seq_len, features)
    y_*_raw: (samples, 1)
    """
    x_scaler = MinMaxScaler()
    y_scaler = MinMaxScaler()

    # fit x_scaler on all timesteps from training only
    X_train_2d = X_train_raw.reshape(-1, X_train_raw.shape[-1])
    x_scaler.fit(X_train_2d)

    # fit y_scaler on training targets only
...
def main():
    ...
    # IMPORTANT: your dataset timestamps look like "01-01-2024 00:00" (day-first)
    df_raw[TIME_COL] = pd.to_datetime(df_raw[TIME_COL], format="%Y-%m-%d %H:%M:%S", errors="raise")
    ...
    # Keep mapping to original row index for aligned export
    df = df.dropna().reset_index(drop=False)  # adds column "index"
    ...
    X_raw = df[FEATURE_COLS].to_numpy(dtype=np.float32)         # (N, F)
    y_raw = df[[TARGET_COL]].to_numpy(dtype=np.float32)         # (N, 1)

    # 4) Build sequences (raw/unscaled)
    X_seq_raw, y_seq_raw, seq_idx_in_df = build_sequences(X_raw, y_raw, SEQ_LEN)

    # Map sequence target rows to df_raw row indices
    ...
    model = Model(inputs=inp, outputs=out, name="lstm_throughput_predictor")
    ...
    model.save(os.path.join(MODEL_DIR, "lstm_predictor.keras"))
    ...
    # 9) Predict full sequence set and export aligned CSV (ALL rows)
    # Transform full X_seq_raw using x_scaler, then predict
    ...
    out_csv = os.path.join(RESULTS_DIR, "cloud_workload_with_predictions_lstm.csv")
    ...
    plt.plot(y_pred_test_inv, label="Predicted (LSTM)", color="orange", linestyle="--")
    plt.xlabel("Test time steps")
    plt.ylabel("Throughput (tasks/sec)")
    plt.title("LSTM Baseline: Actual vs Predicted Throughput")
    plt.legend()
    plt.tight_layout()

    out_png = os.path.join(RESULTS_DIR, "throughput_prediction_lstm.png")
